...
namespace GTRC_Database_API.Controllers
{
    ...
    public class TrackController(TrackService service, BaseService<Track> baseService, FullService<Track> fullService) : BaseController<Track>(baseService, fullService)
    {
        ...
        [HttpPut("Get/ByUniqProps/1")] public async Task<ActionResult<Track?>> GetByUniqProps(TrackUniqPropsDto1 objDto)
        {
            UniqPropsDto<Track> _objDto = new() { Index = 1, Dto = objDto };
            Track? obj = await service.GetByUniqProps(_objDto);
            if (obj is null) { return NotFound(obj); }
            else { return Ok(obj); }
        }

        [HttpPut("Get/ByUniqProps/2")] public async Task<ActionResult<Track?>> GetByUniqProps(TrackUniqPropsDto2 objDto)
        {
            UniqPropsDto<Track> _objDto = new() { Index = 2, Dto = objDto };
            Track? obj = await service.GetByUniqProps(_objDto);
            if (obj is null) { return NotFound(obj); }
            else { return Ok(obj); }
        }

        [HttpPut("Get/ByUniqProps/3")] public async Task<ActionResult<Track?>> GetByUniqProps(TrackUniqPropsDto3 objDto)
        {
            UniqPropsDto<Track> _objDto = new() { Index = 3, Dto = objDto };
            Track? obj = await service.GetByUniqProps(_objDto);
            if (obj is null) { return NotFound(obj); }
            else { return Ok(obj); }
        }
        ...
        [HttpPost("Add")] public async Task<ActionResult<Track?>> Add(TrackAddDto objDto)
        {
            ...
            else
            {
                ...
                UniqPropsDto<Track> uniqPropsDto = new();
        ...
        [HttpPut("Update")] public async Task<ActionResult<Track?>> Update(TrackUpdateDto objDto)
        {
            ...
            else
            {
                ...
                else
                {
                    ...
                    await fullService.UpdateChildObjects(typeof(Track), obj);
                    return Ok(obj);
                }
            }
        }
