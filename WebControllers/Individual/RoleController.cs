...
namespace GTRC_Database_API.Controllers
{
    [ApiController]
    [Route(nameof(Role))]
    public class RoleController(RoleService service, BaseService<Role> baseService, FullService<Role> fullService) : BaseController<Role>(baseService, fullService)
    {
        ...
        {
            Role? objValidated = service.Validate(objDto.Dto2Model());
        ...
        [HttpPut("Update")] public async Task<ActionResult<Role?>> Update(RoleUpdateDto objDto)
        {
            ...
            {
                Role? objValidated = service.Validate(objDto.Dto2Model(obj));
